...
import { APIMessage, GatewayMessageEventExtraFields, Snowflake } from 'discord-api-types/v10'
...
//    guild_id: Snowflake; 
//    author: U.RawUser //	the author of this message (not guaranteed to be a valid user, see below)
...
//    mentions: U.RawUser[]	//array of user objects	users specifically mentioned in the message
...
//    position: number|undefined; //A generally increasing integer (there may be gaps or duplicates) that represents the approximate position of the message in a thread, it can be used to estimate the relative position of the message in a thread in company with total_message_sent on parent thread
...
//core message is a gateway dispatch message btw
...
function from(raw: APIMessage & GatewayMessageEventExtraFields) : CoreMessage {
    
    return {
        content: raw.content,
        channelId: new Id(raw.channel_id),
        guildId: nullableSnowflakeToId(raw.guild_id),
